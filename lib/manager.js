...
Manager.prototype.onHandshake = function (id, data) {
  this.log.debug('[' + process.pid + '] ' + "onHandshake: " + id);
...
Manager.prototype.onConnect = function (id) {
  this.log.debug('[' + process.pid + '] ' + "onConnect: " + id);
...
Manager.prototype.onOpen = function (id) {
  this.log.debug('[' + process.pid + '] ' + "onOpen: " + id);
  this.open[id] = true;
  if(this.knownSockets.indexOf(id) >= 0){
    this.log.debug('[' + process.pid + '] ' + "onOpen: " + id + " called more than once");
    return;
  }else{
    this.log.debug('[' + process.pid + '] ' + "onOpen: " + id + " added to knownSockets");
    this.knownSockets.push(id);
  }
...
Manager.prototype.onDispatch = function (room, packet, volatile, exceptions) {
  this.log.debug('[' + process.pid + '] ' + "onDispatch: room " + room + " exists(" + this.rooms[room] + ")");
  if (this.rooms[room]) {
...
Manager.prototype.onJoin = function (id, name) {
  this.log.debug('[' + process.pid + '] ' + "onJoin: " + id + ", name = " + name);
...
Manager.prototype.onLeave = function (id, room) {
  this.log.debug('[' + process.pid + '] ' + "onLeave: " + id + ", room = " + room);
  if (this.rooms[room]) {
...
Manager.prototype.onClose = function (id) {
  this.log.debug('[' + process.pid + '] ' + "onClose: " + id);
  if (this.open[id]) {
    delete this.open[id];
  }

  if (this.knownSockets.indexOf(id) >= 0) {
    this.log.debug('[' + process.pid + '] ' + "onClose: " + id + ", deleting from knownSockets");
    this.knownSockets.slice(this.knownSockets.indexOf(id), 1);
  }
...
Manager.prototype.onClientDispatch = function (id, packet) {
  this.log.debug('[' + process.pid + '] ' + "onClientDispatch: " + id + ", packet = " + packet);
...
Manager.prototype.onClientMessage = function (id, packet) {
  this.log.debug('[' + process.pid + '] ' + "onClientMessage: " + id + ", packet = " + packet);
...
Manager.prototype.onClientDisconnect = function (id, reason, local) {
  this.log.debug('[' + process.pid + '] ' + "onClientDisconnect: " + id + ", reason = " + reason + ", local = " + local);
...
Manager.prototype.onDisconnect = function (id, local) {
  this.log.debug('[' + process.pid + '] ' + "onDisconnect: " + id + ", local = " + local);
...
Manager.prototype.handleUpgrade = function (req, socket, head) {
  ...
  if (!data) {
    if (this.enabled('destroy upgrade')) {
      socket.end();
      this.log.debug('[' + process.pid + '] ' + 'destroying non-socket.io upgrade');
    }
...
Manager.prototype.authorize = function (data, fn) {
  if (this.get('authorization')) {
    ...
    this.log.debug('[' + process.pid + '] ' + 'client authorized');
    fn(null, true);
  }
