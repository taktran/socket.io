...
Manager.prototype.onOpen = function (id) {
  console.log('[debug] onOpen(%s) called on %s', id, process.pid);
  this.open[id] = true;
  if(this.knownSockets.indexOf(id) >= 0){
    console.log('[debug] onOpen(%s) called more than once on %s', id, process.pid);
    return;
  }else{
    console.log('[debug] onOpen(%s) adding %s to knownSockets on %s', id, id, process.pid);
...
Manager.prototype.onClose = function (id) {
  ...
  if (this.knownSockets.indexOf(id) >= 0) {
    console.log('[debug] onClose called for %s on %d, deleting from knownSockets.', id, process.pid);
  ...
  var self = this;
  console.log('"subscribe" "dispatch:%s" [%s]', id, process.pid);
...
Manager.prototype.onDisconnect = function (id, local) {
  ...
  console.log('"unsubscribe" "dispatch:%s" [%s]', id, process.pid);
  this.store.unsubscribe('dispatch:' + id);
  console.log('"unsubscribe" "message:%s" [%s]', id, process.pid);
  this.store.unsubscribe('message:' + id);
  console.log('"unsubscribe" "disconnect:%s" [%s]', id, process.pid);
...
Manager.prototype.handleClient = function (data, req) {
  ...
  if (handshaken) {
    ...
    if (!this.connected[data.id]) {
      ...
      console.log('"subscribe" "message:%s" [%s]', data.id, process.pid);
      this.store.subscribe('message:' + data.id, function (packet) {
        self.onClientMessage(data.id, packet);
      });
      console.log('"subscribe" "disconnect:%s" [%s]', data.id, process.pid);
